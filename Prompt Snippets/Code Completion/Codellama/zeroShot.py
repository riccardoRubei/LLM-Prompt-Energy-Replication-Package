...
def test(model, tokenizer):
    ...
    {
        "role": "user",
        "content": "public class user(String name){",
    }
    ]
    system_conf = zero_shot_message[0]['content']
    message_conf = zero_shot_message[-1]['content']
    
    #system = "You are an AI assistant specialized in code completion for Java. Your task is to complete the provided Java code segment with one line. Give only the code completion."
    #user = "public static void main("
    #zero_shot_message = f"<s>[INST] <<SYS>>\\n{system}\\n<</SYS>>\\n\\n{user}[/INST]"

    input_ids_conf = tokenizer.apply_chat_template(zero_shot_message,add_generation_prompt=True,return_tensors="pt").to(model.device)

    
    tokenized_chat = torch.tensor(input_ids_conf[0])
    tokenized_chat = tokenized_chat.unsqueeze(0)
    tokenized_chat = tokenized_chat.to(model.device)
    generated_text = model.generate(tokenized_chat, max_new_tokens=512)
    generated_text = tokenizer.decode(generated_text[0], skip_special_tokens=True)
